...
namespace QS.DependencyInjection.Extensions
{
    /// <summary>
    /// 依赖注入拓展类
    /// </summary>
    ...
    public static class DependencyInjectionServiceCollectionExtensions
    {
        ...
        private static IServiceCollection AddAutoScanInjection(this IServiceCollection services)
        {
            ...
            foreach (var type in injectTypes)
            {
                ...
                {
                    RegisterService(services, QS.DependencyInjection.RegisterType.Transient, type, injectionAttribute, canInjectInterfaces);
                }
                // 注册作用域服务
                else if (typeof(IScopeDependency).IsAssignableFrom(type))
                {
                    RegisterService(services, QS.DependencyInjection.RegisterType.Scoped, type, injectionAttribute, canInjectInterfaces);
                }
                // 注册单例服务
                else if (typeof(ISingletonDependency).IsAssignableFrom(type))
                {
                    RegisterService(services, QS.DependencyInjection.RegisterType.Singleton, type, injectionAttribute, canInjectInterfaces);
                }
        ...
        private static void RegisterType(IServiceCollection services, RegisterType registerType, Type type, InjectionAttribute injectionAttribute, Type inter = null)
        {
            ...
            if (registerType == QS.DependencyInjection.RegisterType.Transient)
            {
                RegisterTransientType(services, fixedType, injectionAttribute, fixedInter);
            }

            if (registerType == QS.DependencyInjection.RegisterType.Scoped)
            {
                RegisterScopeType(services, fixedType, injectionAttribute, fixedInter);
            }

            if (registerType == QS.DependencyInjection.RegisterType.Singleton)
            {
