...
namespace QS.ServiceLayer.User
{
    ...
    public class UserService:IUserService,IScopeDependency
    {
        ...
        public UserService(IUserInfo user, IMapper mapper, EFContext context)
        {
            _user = user;
            _mapper = mapper;
            _context = context;
        ...
        public async Task<StatusResult> GetBasicAsync()
        {
            if (!(_user?.Id > 0))
            {
                return new StatusResult("未登录！");
            }

            var data = await _context.Users.FirstOrDefaultAsync(o=>o.Id == _user.Id);
        ...
        public async Task<StatusResult> PageAsync(PageInput<UserEntity> input)
        {
